...
async function assertOwnership(userId: string, chamberId: string) {
  const doctor = await prisma.doctorProfile.findUnique({ where: { userId }, select: { id: true } });
  if (!doctor) return null;
  const chamber = await prisma.doctorChamber.findFirst({ where: { id: chamberId, doctorId: doctor.id } });
  return chamber ? doctor.id : null;
}
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const doctorId = await assertOwnership(session.user.id, id);
  if (!doctorId) return NextResponse.json({ error: "চেম্বার পাওয়া যায়নি" }, { status: 404 });
  ...
  const chamber = await prisma.$transaction(async (tx) => {
    await tx.chamberSchedule.deleteMany({ where: { chamberId: id } });
    return tx.doctorChamber.update({
      where: { id },
      data: {
        nameBn: data.nameBn,
        nameEn: data.nameEn,
